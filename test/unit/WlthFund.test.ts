...
import { Wlth, USDC, WlthFund } from '../../typechain-types';
  ...
  describe('Deployment', () => {
    describe('Success', () => {
      it('Should deploy the contract with initial params', async () => {
        const { wlthFund, owner, deployer, secondarySalesWallet, wlth, usdc} =
          await loadFixture(deployWlthFund);
  ...
  describe('Put Proposal Hash', () => {
    ...
    describe('Reverts', () => {
      ...
      it('Should revert when called by the owner', async () => {
        ...
        await expect(wlthFund.connect(deployer).putProposalHash(proposalId, proposalHash)).to.be.revertedWith('Ownable: caller is not the owner');
      });
    });
  });
  ...
  describe('Put Top 50 Stakers', () => {
    ...
    describe('Reverts', () => {
      ...
      it('Should revert when called by the owner', async () => {
        ...
        await expect(wlthFund.connect(deployer).putTop50Stakers(id, top50StakersList)).to.be.revertedWith('Ownable: caller is not the owner');
      });
    });
  });
  ...
  describe('Fund Investee', () => {
    describe('Success', () => {
      it('should fund the investee', async () => {
        ...
        await expect(wlthFund.connect(owner).fundInvestee(proposalId, investee.address, fundAmount, burnAmount)).to.emit(wlthFund, 'InvesteeFunded')
    ...
    describe('Reverts', () => {
      it('Should revert when investee already funded', async () => {
        ...
        await expect(wlthFund.connect(owner).fundInvestee(proposalId, investee.address, fundAmount, burnAmount)).to.be.revertedWithCustomError(
          wlthFund,
          'WlthFund__InvesteeAlreadyFunded'
        );
      });
      ...
      it('Should revert when not called by owner', async () => {
        ...
        await expect(wlthFund.connect(deployer).fundInvestee(proposalId, investee.address, fundAmount, burnAmount)).to.be.revertedWith('Ownable: caller is not the owner');
      });
    });
    });
  });
