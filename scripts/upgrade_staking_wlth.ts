...
import { StakingWlth } from '../typechain-types';
import { confirm, getEnvByNetwork, upgradeContract, verifyContract } from './utils';

async function main() {
  const [deployer] = await ethers.getSigners();
  const contractName = 'StakingWlth';
  const contractToUpgrade = getEnvByNetwork('STAKING_WLTH_CONTRACT', network.name) as string;
